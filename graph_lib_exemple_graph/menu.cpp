...
MenuInterface::MenuInterface(int x, int y, int w, int h, int choix)
{
    if (choix==1)
    {
        ...
        m_texteButton1.set_message("Simuler");
        ...
        m_texteButton2.set_message("Instruction");
    ...
    if(choix == 3)
    {
        ...
        m_top_box.add_child(m_background);
        m_background.set_pic_name("background.jpg");
        m_top_box.add_child(m_img_regle);
        m_img_regle.set_pic_name("regle.jpg");
        m_img_regle.set_gravity_xy(grman::GravityX::Center,grman::GravityY::Center);
    }
